...
#include "/root/mpquic_1155210162/xquic/src/transport/scheduler/xqc_scheduler_sttf.h"
...
int g_enable_reschedule = 0;
int g_spec_local_addr = 0;
int g_mpshell = 0;
int g_endless_sending = 0;
/* Marks: for video streaming */
int g_video_stream = 0;
char* filename;
FILE* file;
int FRAME_INT;
char line[MAX_LINE_LENGTH];
char nextLine[MAX_LINE_LENGTH];
int frame_index, frame_size;
int video_finished = 0;
long int offset;
char *send_frame_body;
size_t send_body_len;
struct event *ev_next_frame;
int first_frame = 1;
int g_enable_fec = 0;
...
xqc_server_set_event_timer(xqc_usec_t wake_after, void *user_data)
{
    ...
    /* Marks: timer in event is not accurate enough, use this to make it better */
    tv.tv_usec = (tv.tv_sec==0 && wake_after % 1000000<1000) ? 0 : wake_after % 1000000;
...
/* Marks: simulate video streaming */
typedef struct Node {
    int item;
    struct Node* next;
} Node;
...
ItemList itemList = {NULL, NULL, PTHREAD_MUTEX_INITIALIZER, PTHREAD_COND_INITIALIZER};

// Function to add an item to the list
void add_item(int item, int tail) {
    ...
    if (itemList.head == NULL) {
        ...
        itemList.tail = newNode;
    } else if (tail) {
        // Append the new node at the end
        itemList.tail->next = newNode;
        itemList.tail = newNode;
    } else{
        // Append the new node at the start
        newNode->next = itemList.head;
        itemList.head = newNode;
    }
    // Signal the condition variable that a new item has been added
    pthread_cond_signal(&itemList.cond);
    pthread_mutex_unlock(&itemList.mutex);
}

// Function to process and remove the earliest item from the list
void process_item(int* itemValue, int* noFrame, int* firstFrame) {
    pthread_mutex_lock(&itemList.mutex);
    // Wait for an item to be available
    if (*firstFrame == 1){
        while (itemList.head == NULL) {
            pthread_cond_wait(&itemList.cond, &itemList.mutex);
        }
        *firstFrame = 0;
    }
    // if (*firstFrame == 1){
    ...
    *noFrame = 0;
    ...
    // Record the item value
    *itemValue = temp->item;
    pthread_mutex_unlock(&itemList.mutex);
    // Free the node after recording the value
    free(temp);
}

// Thread function for processing items
void* consumer_thread(void* arg) {
    void** args = (void**)arg;         // Cast the input argument to a void pointer array
    xqc_stream_t* stream = (xqc_stream_t*)args[0];       // Extract h3_request
    user_stream_t* user_stream = (user_stream_t*)args[1]; // Cast the input argument to user_stream_t pointer
    printf("1---Address of h3_request: %p\n", (void*)stream);
    printf("1---Address of user_stream: %p\n", (void*)user_stream);
    ssize_t ret;
    // free(user_stream->send_body);
    user_stream->send_body = NULL;
    
    int itemValue;  // Value from the list
    int noFrame;
    int firstFrame;
    do {
        // Retrieve the next item
        process_item(&itemValue, &noFrame, &firstFrame);  // This should be implemented as per your earlier design
        if (noFrame){
            printf("no frame\n");
            break;
        }
        printf("itemValue: %d\n", itemValue);
        // Check for the finish signal
        if (itemValue == -1) {
            ret = xqc_stream_send(stream, NULL, 0, 1);
            break;  // Exit if finish signal is processed
        }
        // Set up the user stream based on itemValue
        if (user_stream->send_body == NULL) {
            // printf("prepare to create send_body\n");
            user_stream->send_body_max = MAX_BUF_SIZE;
            user_stream->send_body_len = itemValue;  // Use itemValue as the real send size
            user_stream->send_body = malloc(user_stream->send_body_len);
            memset(user_stream->send_body, 0, user_stream->send_body_len);
            if (user_stream->send_body == NULL) {
                perror("Failed to allocate memory for send body");
                return NULL;  // Handle memory allocation failure
            }
            // Initialize the send body with some data if needed
            // For example: memset(user_stream->send_body, 0, user_stream->send_body_len);
            printf("create responce, real_send_size: %zu\n", user_stream->send_body_len);
            user_stream->send_offset = 0;  // Reset offset for new body
        }
        ...
        if (ret < 0) {
            ...
            return NULL;  // Handle send error
        } else {
            user_stream->send_offset += ret;  // Update the offset
            printf("Sent: %zd, new offset: %zu\n", ret, user_stream->send_offset);
        }
        // Clean up after processing if the entire body has been sent
        free(user_stream->send_body);  // Free the allocated memory
        user_stream->send_body = NULL;  // Reset for the next item
    } while (0); 
...
int
xqc_server_stream_send_frame(xqc_stream_t *stream)
{
    ssize_t ret;
    static int line_read = 0;
    static char line[256];
    static int frame_index, frame_size;
    ...
    if (line_read == 0) {
        if (fgets(line, sizeof(line), file) != NULL) {
            if (sscanf(line, "%d,%d", &frame_index, &frame_size) == 2) {
                line_read = 1;
            } else {
                return 0;
            }
            ...
            ret = xqc_stream_send(stream, NULL, 0, 1);
            printf("xqc_stream_send video end\n");
            return ret;
        }
    }

    size_t send_body_len = frame_size;
    void *send_frame_body = malloc(send_body_len);
    size_t offset = MAX_BUF_SIZE - send_body_len;

    if (offset > 0) {
        ret = xqc_stream_send(stream, send_frame_body, send_body_len, 0);
    } else {
        printf("send_body_max is not enough\n");
        free(send_frame_body);
        return 0;
    }
...
xqc_server_stream_create_notify(xqc_stream_t *stream, void *user_data)
{
    ...
    printf("stream=%p, user_stream=%p, user_stream->stream=%p\n", (void*)&stream, (void*)&user_stream, (void*)&(user_stream->stream));
...
void
xqc_server_send_next_frame(int fd, short what, void *arg)
{
    xqc_stream_t *stream = (xqc_stream_t *) arg;
    // printf("send next frame\n");
    ssize_t ret;
    ret = xqc_server_stream_send_frame(stream);
    if (ret > 0 || ret == -XQC_EAGAIN) {
        struct timeval tv;
        tv.tv_sec = 0;
        tv.tv_usec = FRAME_INT;
        event_add(ev_next_frame, &tv);
    }
}
...
xqc_server_stream_write_notify(xqc_stream_t *stream, void *user_data)
{
    ...
    if (g_video_stream){
        ...
        ret = xqc_server_stream_send(stream, user_data);
    }
    return ret;
}

/* Marks: Thread function for adding items from a file */
void* producer_thread(void* arg) {
    void** args = (void**)arg;         // Cast the input argument to a void pointer array
    xqc_stream_t* stream = (xqc_stream_t*)args[0];       // Extract h3_request
    user_stream_t* user_stream = (user_stream_t*)args[1]; // Cast the input argument to user_stream_t pointer
    // const char* filename = (const char*)arg;
    const char* filename = "/root/develop/original/xquic/frame_info_24mbps_30fps_2.txt"; // frame_1080 frame_info_short frame_2160_5min frame_2160_1min frame_info_exo frame_info_24mbps_30fps
    int FRAME_INT = 33667;
    ...
    ssize_t ret;
    int itemValue;  // Value from the list
    user_stream->send_body_max = 1024*1024;
    user_stream->send_body = malloc(user_stream->send_body_max);
    while (1) {
        // Check if there's a new line to read
        if (fgets(line, sizeof(line), file)) {
            // Use sscanf to parse the line
            if (sscanf(line, "%d,%d", &index, &value) == 2) {
                itemValue = value;
                // Set up the user stream based on itemValue
                user_stream->send_body_len = itemValue;  // Use itemValue as the real send size
                clock_gettime(CLOCK_MONOTONIC, &start_time);
                ret = xqc_stream_send(stream, user_stream->send_body + (user_stream->send_body_max-user_stream->send_body_len), 
        ...
        if (fgets(nextLine, sizeof(nextLine), file) != NULL) {
            fseek(file, -strlen(nextLine), SEEK_CUR);
            elapsed_time = (end_time.tv_sec - start_time.tv_sec)*1e6 + (end_time.tv_nsec - start_time.tv_nsec) / 1e3;
            if (ret == -XQC_EAGAIN){
                usleep((int)(2 * FRAME_INT-elapsed_time));
            } else {
                usleep((int)(FRAME_INT-elapsed_time));
            }
        } else {
            printf("End of file\n");
            ret = xqc_stream_send(stream, NULL, 0, 1);
    ...
    fclose(file);
    xqc_server_stream_close_notify(stream, user_stream);
...
xqc_server_stream_read_notify(xqc_stream_t *stream, void *user_data)
{
    ...
    if (fin) {
        if (g_video_stream){
            ev_next_frame = event_new(eb, -1, 0, xqc_server_send_next_frame, stream);
        }
        xqc_server_stream_write_notify(stream, user_data);
    }
    printf("xqc_server_stream_read_notify\n");
    return 0;
}
...
xqc_server_h3_conn_close_notify(xqc_h3_conn_t *h3_conn, const xqc_cid_t *cid, void *conn_user_data)
{
    ...
    FILE *file = fopen("log_marks/run_test_server_output.txt", "a");
    if (file) {
        fprintf(file, "%s, send_count:%u, lost_count:%u, spurious_lost_count:%u, tlp_count:%u, recv_count:%u, srtt:%"PRIu64" min_rtt:%"PRIu64" early_data_flag:%d, conn_err:%d, ack_info:%s, alpn:%s\n",
                timeString, stats.send_count, stats.lost_count, stats.spurious_loss_count, stats.tlp_count, stats.recv_count, stats.srtt, stats.min_rtt, stats.early_data_flag, stats.conn_err, stats.ack_info, stats.alpn);
        fclose(file);
    }
...
"   -z:   Scheduler. m:minrtt v:minrcvt. e:ECF\n"
"   -A    Enable more accurate bandwidth estimate without app-limit..\n"
"   -R    Enable reinjection. Default is 0, no reinjection, > 4 (4*: reinjection with timer, alpha=1.*).\n"
, prog);
}
...
int main(int argc, char *argv[]) {
    ...
    int ch = 0;
    while ((ch = getopt_long(argc, argv, "a:p:efc:Cs:w:r:l:Az:u:x:6bS:MR:o:EK:mLVQ:U:yHD", long_opts, NULL)) != -1) {
        switch (ch) {
        case 'H':
        ...
        case 'L':
            ...
            break;
        case 'V':
            printf("option video streaming: %s\n", "on");
            g_video_stream = 1;
            break;
        case 'C': /* Pacing on. */
        ...
        case 'M':
            ...
            break;
        case 'A': /* Enable accurate bandwidth estimate in BBR. */
            printf("option modified bbr :%s\n", "on");
            modified_BBR = 1;
            break;
        case 'z': /* scheduler. m: minrtt, v: minrcvt, e: ECF */
        ...
        case 'R':
            ...
            break;
        case 'D':
            printf("option reschedule: enable\n");
            g_enable_reschedule = 1;
            break;
        case 'o':
    ...
    /* Marks: get video frame info */
    if (g_video_stream){
        // frame_info_24mbps_30fps_8 frame_info_2079217_30fps_no_bframe_4 frame_info_2079217_30fps_no_bframe_2 frame_info_2079217_30fps_no_bframe
        filename = "/root/develop/original/xquic/frame_info_2079217_30fps_no_bframe_8.txt";
        FRAME_INT = 33667;
...
//            .enable_accurate_bw_estimate = modified_BBR,
        },
...
//        .mp_enable_reschedule = g_enable_reschedule,
//        .resche_alpha = resche_alpha,
    };
    ...
    switch (c_sche){
//        case 'm': conn_settings.scheduler_callback = xqc_minrtt_scheduler_cb;break;
        case 's': conn_settings.scheduler_callback = xqc_sttf_scheduler_cb;break;
        default: conn_settings.scheduler_callback = xqc_sttf_scheduler_cb;
    }
    ...
    if (g_enable_reinjection == 1) {
        ...
        conn_settings.reinj_ctl_callback    = xqc_deadline_reinj_ctl_cb;
        if (g_enable_reinjection == 4) {
//            conn_settings.reinj_alpha = 14;
        } else if (g_enable_reinjection == 5) {
//            conn_settings.reinj_alpha = 0;
        } else{
//            conn_settings.reinj_alpha = 10 + g_enable_reinjection%40;
        }
    }
